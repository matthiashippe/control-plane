...
## Status
DONE (2026-09-18 22:52, Verifier PASS)
...
## Done Condition
- [x] `pnpm test` grün, `test/register.test.ts` mit mindestens 6 Tests: gültige Registrierung
...
- [x] `pnpm e2e` grün (kompletter Erstlauf)
...
- [x] Alle bisherigen E2E-Läufe bleiben grün
...
## Acceptance Criteria
- [x] Register gemäß docs/protocol.md: `payload_hash` = keccak256 des JSON der alphabetisch
...
- [x] Tabelle `automatons` (id, address, creator, name, bio, genesis_prompt_hash, registered_at);
      Antwort `{ automaton: { automaton_id, automaton_address, creator_address, name, bio, registered_at } }`
- [x] `GET /v1/credits/pricing` -> `{ tiers: [], topup_tiers_usd: [5, 25, 100, 500, 1000, 2500] }`
- [x] `POST /v1/credits/transfer` und `/v1/credits/transfers` -> `501 { error: "not_implemented",
      reason: ... }` (Entscheidung in STATE.md); kein Transfer-Code
- [x] `harness/e2e/full.sh` zählt `Conway API error`- und `registration failed`-Zeilen im
      Runtime-Log und prüft `Automaton identity registered.`
- [x] docs/protocol.md Abschnitt Registry um die serverseitige Prüfung ergänzt
...
- 2026-09-18 22:52 Verifier (Sonnet): PASS. Eigene Läufe: 38/38 Unit, E2E OK turns=5 registered=true api_errors=0, INFERENCE OK, TOPUP OK, SMOKE OK, ps -q leer; Hash- und Signaturaufbau gegen den Runtime-Client d8f8168 verglichen.
