...
## Status
DONE (2026-09-18 22:28, Verifier PASS)
...
## Done Condition
- [x] `pnpm test` grün, `test/inference.test.ts` mit mindestens 6 Tests: Proxy antwortet im
...
- [x] Topup- und Smoke-Test bleiben grün
...
## Acceptance Criteria
- [x] Saldo intern in Millicents (`balance_mc`, `delta_mc`), API weiter `balance_cents` =
      floor(mc / 1000); Topup 5 USD = 500 000 mc. Tests aus Goal 1 und 2 auf Millicents angepasst,
      ohne Assertions abzuschwächen
- [x] `ChatProvider`-Interface (`id`, `models()`, `chat(req)`) in `src/inference/provider.ts`;
...
- [x] `POST /v1/chat/completions`: Header `Authorization: <key>` roh; Body OpenAI-Format mit
...
- [x] `GET /v1/models`: `{ data: [{ id, provider, available: true, pricing: { input_per_million,
      output_per_million } }] }` mit Verkaufspreisen (Listenpreis x 1,3)
- [x] Harness: Runtime-Konfiguration `inferenceModel: "mock-1"` in setup.json; `inference.sh`
      wartet auf fünf `Turn`-Zeilen im Runtime-Log, liest Saldo und Ledger und prüft Konsistenz
- [x] `docs/protocol.md` Abschnitt Inferenz um Millicents und Vorprüfung ergänzt
- [x] Katalog-Aliase (`CP_MODEL_ALIASES`): die Runtime fragt für Agent-Turns `gpt-5.2`/`gpt-5-mini`
...
- 2026-09-18 22:28 Verifier (Sonnet): PASS. Eigene Läufe: 30/30 Unit, INFERENCE OK turns=5 balance_cents=489 inference_rows=5 ledger_consistent=true, TOPUP OK topup_mc=500000, SMOKE OK, ps -q leer; Scope und Betrugscheck sauber.
