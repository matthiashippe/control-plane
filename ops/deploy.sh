...
#
# "Vor jedem Deploy, ohne Ausnahme: pnpm test gruen, und bei jeder Aenderung am Laufzeitpfad auch
# pnpm e2e gruen." That has been the rule since 19.09. and `deploy/rollout.sh` never checked it,
# because `deploy/**` is not touched without a human. So the rule lived entirely in whoever was
# typing, and on 2026-09-22 that failed: a cycle read "1 failed | 465 passed", took it for the
# output of a counter-proof it had just run, and rolled out anyway. The failing test was the one
# holding docs/bounties.md against the MCP server's own schema, and the deploy was harmless by
