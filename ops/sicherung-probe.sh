...
WAHL="tail -1"
[[ "${1:-}" == "--oldest" ]] && WAHL="head -1"

ssh -i "$KEY" -o BatchMode=yes -o ConnectTimeout=10 "$HOST" bash -s <<REMOTE
set -euo pipefail
quelle=\$(ls -1 /opt/control-plane/backups/cp-*.db 2>/dev/null | sort | $WAHL)
[[ -n "\$quelle" ]] || { echo "FAILED  no backup in /opt/control-plane/backups"; exit 1; }
echo "probe:  \$(basename "\$quelle"), \$(stat -c %s "\$quelle") bytes, written \$(stat -c %y "\$quelle" | cut -d. -f1)"

arbeit=\$(mktemp -d)
trap 'rm -rf "\$arbeit"; docker rm -f cp-restore-probe >/dev/null 2>&1 || true' EXIT
cp "\$quelle" "\$arbeit/cp.db"
chmod 666 "\$arbeit/cp.db"
...
echo "ok      /health on the restored copy: \$antwort"

status=\$(hole /v1/status)
