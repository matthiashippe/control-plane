 ...
 */
function listFromReport(name: string): string[] {
  const source = readFileSync("ops/db-report.cjs", "utf-8")
    .split("\n")
    .filter((line) => !line.trimStart().startsWith("//"))
    .join("\n");
  const start = source.indexOf(`const ${name} = [`);
  expect(start, `${name} is not in ops/db-report.cjs any more`).toBeGreaterThan(-1);
...
describe('"ours" means the same in the service as in the report', () => {
  it("has the same key name patterns on both sides", () => {
    expect([...OUR_KEY_NAMES].sort()).toEqual(listFromReport("OUR_KEY_NAMES").sort());
  });

  it("has the same hardcoded addresses on both sides", () => {
    expect([...OUR_ADDRESSES].sort()).toEqual(listFromReport("OURS").sort());
  });
  ...
  it("finds an address by its key name and not only by the list", () => {
    ...
    for (const [address, name] of [[seed, "ops-seed-vera"], [stranger, "conway-automaton"]]) {
    ...
    const found = ourAddresses(db, [seed, stranger, OUR_ADDRESSES[0]]);
    expect(found.has(seed), "a seed agent is ours by its key name").toBe(true);
    expect(found.has(OUR_ADDRESSES[0]), "the operator wallet is ours by the list").toBe(true);
    ...
    expect(found.has(stranger), "a real runtime's key name is not ours to claim").toBe(false);
  });
