...
import { formalCheck, hoursStillOpen } from "../ops/award.js";
...
describe("the formal check against a brief", () => {
  it("stays quiet on work that keeps to it", () => {
    expect(formalCheck(BRIEF, "A short honest line about the market for agents here.")).toEqual([]);
  });

  it("counts words against the limit the brief names", () => {
    const b = formalCheck(BRIEF, "One two three four five six seven eight nine ten eleven twelve.");
    expect(b).toHaveLength(1);
    expect(b[0]).toContain("12 words against a limit of 10");
  });

  it("finds each word the brief rules out, and only those", () => {
    expect(formalCheck(BRIEF, "A seamless line.")).toEqual([
    ...
    expect(formalCheck(BRIEF, "A line about solutions.")).toEqual([]);
  });

  it("catches punctuation the brief forbids", () => {
    expect(formalCheck(BRIEF, "A short line!")).toEqual(["has an exclamation mark"]);
    expect(formalCheck(BRIEF, "A short line — with a dash.")).toEqual(["has an em or en dash"]);
  });

  it("counts sentences when the brief asks for one", () => {
    const b = formalCheck(BRIEF, "One line here. Another line there.");
    ...
    expect(formalCheck(BRIEF, "One line here; another line there.")).toEqual([]);
  });

  it("reports everything at once rather than stopping at the first", () => {
    const b = formalCheck(BRIEF, "A seamless platform solution — truly! It goes on and on and on and on.");
    expect(b).toHaveLength(7);
  });

  it("says nothing when the brief sets no measurable rule", () => {
    expect(formalCheck("Write something nice about us.", "Something! With a dash — and more.")).toEqual([]);
  });
});
...
describe("hours left on a job", () => {
  ...
  it("counts the hours while the job is open", () => {
    ...
    expect(hoursStillOpen("2026-09-23T17:47:50.175Z", jetzt)).toBeCloseTo(24.46, 1);
  });
  it("is zero once the deadline has passed, which is what lets an award through", () => {
    expect(hoursStillOpen("2026-09-22T17:19:00Z", jetzt)).toBe(0);
    expect(hoursStillOpen("2026-09-01T00:00:00Z", jetzt)).toBe(0);
  });
  it("is NaN and not zero when the deadline cannot be read", () => {
    ...
    expect(hoursStillOpen("tomorrow afternoon", jetzt)).toBeNaN();
    expect(hoursStillOpen("", jetzt)).toBeNaN();
  });
});
