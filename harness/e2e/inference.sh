#!/usr/bin/env bash
# Goal 3: Nach dem Bootstrap-Topup fährt die Runtime fünf Turns gegen den Mock-Provider;
# jeder Call ist eine Ledger-Zeile und der Saldo stimmt mit dem Ledger überein.
set -euo pipefail
cd "$(dirname "$0")/.."
...
[[ -n "$key" && -n "$wallet" ]] || { echo "$out"; echo "INFERENCE FAIL: Key oder Wallet fehlt"; exit 1; }
echo "wallet=$wallet key_prefix=${key:0:15}"
"${COMPOSE[@]}" run --rm chain-tools fund "$wallet" 6 >/dev/null

echo "--- Runtime starten, auf fünf Turns warten"
...
printf '%s\n' "$logs" | grep -q 'Bootstrap topup: +\$5' || { echo "INFERENCE FAIL: kein Bootstrap-Topup"; exit 1; }
(( turns >= 5 )) || { echo "INFERENCE FAIL: nur $turns Turns"; printf '%s\n' "$logs" | tail -60; exit 1; }

echo "--- Saldo über die API"
...
[[ "$remaining" == "0" ]] || { echo "INFERENCE FAIL: Container übrig"; exit 1; }
