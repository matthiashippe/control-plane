#!/usr/bin/env bash
# Acceptance stage 2: the unmodified upstream runtime as a container ON THE VM against the
# public control plane (https://cp.hippe.eu), with the throwaway wallet from stage 1 as the
# runtime wallet. First run: provisioning, registration, bootstrap topup of 5 USD over PayAI,
# turns over OpenRouter, sleep. Needs 5 USDC on the throwaway wallet (Base). Nothing runs locally.
#   pnpm e2e:prod
set -euo pipefail
cd "$(dirname "$0")/.."
...
[[ -f "$WALLET_FILE" ]] || { echo "PROD FAIL: $WALLET_FILE missing (run pnpm e2e:mainnet first)"; exit 2; }
...
echo "throwaway wallet $addr, USDC on Base: $usdc"
python3 -c "import sys; sys.exit(0 if float('$usdc') >= 5 else 1)" || { echo "PROD FAIL: at least 5 USDC needed (the runtime bootstrap topup buys 5 USD)"; exit 1; }

echo "--- build the runtime image on the VM"
"${SSH[@]}" 'mkdir -p /opt/control-plane/acceptance/runtime'
rsync -az -e "ssh -i $KEY -o StrictHostKeyChecking=accept-new" runtime/ "$HOST:/opt/control-plane/acceptance/runtime/"
"${SSH[@]}" 'cd /opt/control-plane/acceptance/runtime && docker build -q -t acceptance-runtime . >/dev/null && echo image ok'

echo "--- wallet and setup into a fresh volume, start the first run"
"${SSH[@]}" bash -s "$pk" "$CP_URL" <<'REMOTE'
set -euo pipefail
PK="$1"; CPURL="$2"
docker rm -f acceptance >/dev/null 2>&1 || true
docker volume rm -f acceptance-home >/dev/null 2>&1 || true
docker volume create acceptance-home >/dev/null
# Wallet file in the runtime's own format (identity/wallet.ts), owned by the image's automaton user.
UID_RT=$(docker run --rm --entrypoint id acceptance-runtime -u)
docker run --rm -v acceptance-home:/home/automaton -e PK="$PK" -e UID_RT="$UID_RT" alpine sh -c '
...
docker run --rm -v acceptance-home:/home/automaton -v /opt/control-plane/acceptance/runtime/setup.prod.json:/setup.json:ro \
  -e CONWAY_API_URL="$CPURL" acceptance-runtime provision 2>&1 | grep -v privateKey | tail -3
docker run -d --name acceptance -v acceptance-home:/home/automaton -e CONWAY_API_URL="$CPURL" acceptance-runtime run >/dev/null
echo "runtime started"
REMOTE

echo "--- wait for registration, topup, turns and sleep (at most 6 minutes)"
deadline=$((SECONDS + 360))
while (( SECONDS < deadline )); do
  logs=$("${SSH[@]}" 'docker logs acceptance 2>&1' || true)
...
echo "--- clean up"
"${SSH[@]}" 'docker rm -f acceptance >/dev/null 2>&1; docker volume rm -f acceptance-home >/dev/null 2>&1; docker rmi -f acceptance-runtime >/dev/null 2>&1; rm -rf /opt/control-plane/acceptance; echo clean'
