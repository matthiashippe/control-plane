#!/usr/bin/env bash
# Goal 2: on startup the runtime buys 5 USD of credits over /pay (x402) with USDC on Anvil.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "--- put the USDC mock on the chain"
...
[[ -n "$key" && -n "$wallet" ]] || { echo "$out"; echo "TOPUP FAIL: key or wallet missing"; exit 1; }
echo "wallet=$wallet key_prefix=${key:0:15}"

echo "--- 6 USDC onto the wallet"
"${COMPOSE[@]}" run --rm chain-tools fund "$wallet" 6

echo "--- start runtime, wait for the bootstrap topup"
...
printf '%s\n' "$logs" | grep -q 'Bootstrap topup: +\$5' || { echo "TOPUP FAIL: no 'Bootstrap topup: +\$5' in the runtime log"; printf '%s\n' "$logs" | tail -40; exit 1; }
...
echo "--- USDC balances on the chain"
...
[[ "$remaining" == "0" ]] || { echo "TOPUP FAIL: containers left behind"; exit 1; }
# Since goal 3 the runtime starts turning right after the topup, so the balance ends up just
# under 500. The topup credit itself has to be exactly 500 000 mc in exactly one ledger row.
