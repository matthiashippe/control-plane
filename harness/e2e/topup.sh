...
ledger_rows=$("${COMPOSE[@]}" exec -T cp node -e '
  ...
  const r = db.prepare("SELECT count(*) AS n, coalesce(sum(delta_cents),0) AS sum FROM ledger WHERE kind = ?").get("topup");
  const p = db.prepare("SELECT status, tx_hash FROM payments").all();
  console.log(JSON.stringify({ rows: r.n, sum: r.sum, payments: p }));' 2>&1)
echo "$ledger_rows"
...
[[ "$balance_cents" == "500" && "$rows" == "1" ]] || { echo "TOPUP FAIL: balance_cents=$balance_cents ledger_rows=$rows"; exit 1; }
echo "TOPUP OK balance_cents=$balance_cents ledger_rows=$rows"
